...
- `PATCH /api/bikes/[bikeId]/status` validates a bearer Supabase token, loads the target bike with a service-role client, enforces transition rules, updates status/active rider fields, and records a `bike_status_events` row (`src/app/api/bikes/[bikeId]/status/route.ts`).
