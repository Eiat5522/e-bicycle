...
- API routes under `/api/bikes/[bikeId]` support authenticated status updates and admin-only status event reads for mobile/admin workflows (`src/app/api/bikes/[bikeId]/status/route.ts`, `src/app/api/bikes/[bikeId]/status-events/route.ts`).
...
- `SUPABASE_SERVICE_ROLE_KEY` for privileged admin operations such as user creation and status mutations through server/API code
