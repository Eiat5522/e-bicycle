...
- Server mutations use either `requireAdmin()`/`requireAdminForAction()` or explicit bearer-token validation plus an admin profile check.
- API status mutation validates the bearer token but does not require `is_admin`; it enforces active-rider transition rules instead. The status-events read route does require admin profile access.
...
The status API writes these events after successful status transitions. Dashboard active-ride recovery reads recent `ride_start` events for currently in-use bikes. Bicycle detail and the status-events API expose event history.
...
- Bike status API mutation/read paths that need to bypass row-level policies after validating the caller (`src/app/api/bikes/[bikeId]/status/route.ts`, `status-events/route.ts`).
