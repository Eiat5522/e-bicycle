...
beforeAll(() => {
  ...
  mockFrom.mockImplementation((table: string) => {
    ...
    if (table === "sustainability_reporting") {
      return {
        select: jest.fn().mockReturnValue({
          order: jest.fn().mockResolvedValue({ data: [{ id: "esg-1" }], error: null }),
        }),
      };
    }
...
describe("/api/reports/sustainability", () => {
  beforeEach(() => {
    isAdmin = true;
    mockRpc.mockReset();
  });
  ...
  it("lists existing reports on GET", async () => {
    ...
    expect(body.reports).toEqual([{ id: "esg-1" }]);
  });

  it("generates a sustainability report on POST", async () => {
    mockRpc.mockResolvedValue({
      data: { id: "esg-2", carbon_reduced_kg: 12.5 },
      error: null,
    });
    ...
    expect(body.report.id).toBe("esg-2");
    expect(mockRpc).toHaveBeenCalledWith("generate_sustainability_report", {
