...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'bikes_rate_per_minute_nonnegative'
  ) then
    alter table public.bikes
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'bike_ride_history_rate_per_minute_nonnegative'
  ) then
    alter table public.bike_ride_history
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'bike_ride_history_billable_minutes_nonnegative'
  ) then
    alter table public.bike_ride_history
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'bike_ride_history_currency_code_format'
  ) then
    alter table public.bike_ride_history
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'bike_ride_history_total_cost_nonnegative'
  ) then
    alter table public.bike_ride_history
